...
int main(){
	...
	// Ingresamos a Sapo y Sepo
    cin >> pts[n] >> pts[n+1];
	Point<ll> Sapo, Sepo;
	Sapo = pts[n];
	Sepo = pts[n+1];

	// Armamos el Convex Hull: Puntos + Sapo + Sepo
	vector <Point<ll>> hull = convex_hull(pts);
	double distancia = 0;
	auto it1 = find(hull.begin(), hull.end(), Sapo);
    auto it2 = find(hull.begin(), hull.end(), Sepo);
	// Ambos estan en el Convex Hull
	if (it1 != hull.end() && it2 != hull.end()){
		double arriba = 0;
        double abajo = 0;
		int idxSapo, idxSepo;
		for (int i = 0; i < hull.size(); i++){
			if (hull[i] == Sapo) idxSapo = i;
			if (hull[i] == Sepo) idxSepo = i;
		}
        int i = idxSapo;
		int hullSize = hull.size();
		while (i != idxSepo){
			abajo += (hull[i] - hull[(i - 1 + hullSize) % hullSize]).dist();
			i = (i - 1 + hullSize) % hullSize;
		}
		i = idxSapo;
		while (i != idxSepo){
			arriba += (hull[i] - hull[(i + 1 ) % hullSize]).dist();
			i = (i + 1) % hullSize;
		}


        distancia = min(abajo,arriba);
	}
	else{
		distancia = (Sapo-Sepo).dist();
	}
	cout << fixed << setprecision(15) << distancia;
	return 0;
}
